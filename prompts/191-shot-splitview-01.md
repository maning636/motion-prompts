...
【动效名称】前后对比拉杆
【分类】镜头动效
【用途】处理前后两版叠放，分割杆先猛甩过冲再慢扫回，杆过处新版显影揭出，AI 效果对比段标配
【标签】对比 / 滑杆 / 前后
【画幅】1920×1080（16:9 横屏）
【时长】5 秒，动画确定性、首尾可循环

【视觉与设计规范】
处理前后两版叠放，分割杆先猛甩过冲再慢扫回，杆过处新版显影揭出，AI 效果对比段标配。
【可改文案】卡片标题（默认「周报生成效果」）、前角标（默认「BEFORE」）、后角标（默认「AFTER」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
