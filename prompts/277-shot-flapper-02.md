...
【动效名称】逐字裂升
【分类】镜头动效
【用途】标题逐字从裁切线下滑升、带 10% 过冲回落（每字间隔 2f），末字强调色收尾——琴键式利落入场
【标签】标题 / 入场 / 文字
【画幅】1920×1080（16:9 横屏）
【时长】4.5 秒，动画确定性、首尾可循环

【视觉与设计规范】
标题逐字从裁切线下滑升、带 10% 过冲回落（每字间隔 2f），末字强调色收尾——琴键式利落入场。
【可改文案】标题（默认「集结这一拍」）、副标题（默认「每个字都滑升到位」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
