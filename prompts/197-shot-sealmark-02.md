...
【动效名称】品牌墨印开场
【分类】镜头动效
【用途】十字准星描画→字标逐字压印（scale+blur 入场三件套）→打字机副标→静止满一秒再上浮消散，低能量品牌片头
【标签】品牌 / 开场 / 字标
【画幅】1920×1080（16:9 横屏）
【时长】3 秒，动画确定性、首尾可循环

【视觉与设计规范】
十字准星描画→字标逐字压印（scale+blur 入场三件套）→打字机副标→静止满一秒再上浮消散，低能量品牌片头。
【可改文案】字标（默认「MOTIONLIB」）、副标（默认「镜头动效素材库」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
