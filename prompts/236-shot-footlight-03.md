...
> 分类：镜头动效 ｜ 时长：5.3s ｜ 在线预览：https://maning789link.top/station.html
...
【动效名称】聚光扫字揭示
【分类】镜头动效
【用途】暗场里几乎不可见的标题被锥形光摆动扫亮——双层同文本，亮版随光斑走，两个来回后全亮定格。
【标签】光效 / 标题 / 暗场
【画幅】1920×1080（16:9 横屏）
【时长】5.3 秒，动画确定性、首尾可循环

【视觉与设计规范】
暗场里几乎不可见的标题被锥形光摆动扫亮——双层同文本，亮版 radial mask 随光斑走，两个来回后全亮定格。
【可改文案】暗场标题（默认「暗夜里的光」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
