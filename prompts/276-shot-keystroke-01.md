...
【动效名称】打字搜索穿透
【分类】镜头动效
【用途】搜索框 3f/字符真人速度打字、非目标卡按阅读序错峰淡出、目标卡滑位浮起、双圈 ripple 点击后相机推进穿透
【标签】交互 / 搜索 / UI
【画幅】1920×1080（16:9 横屏）
【时长】5 秒，动画确定性、首尾可循环

【视觉与设计规范】
搜索框 3f/字符真人速度打字、非目标卡按阅读序错峰淡出、目标卡滑位浮起、双圈 ripple 点击后相机推进穿透。
【可改文案】搜索词（默认「nano-lab」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
