...
【动效名称】字标描画结晶
【分类】镜头动效
【用途】大字距骨架字标全字符并行连续描画，同帧起笔、同帧齐收成词——整词一体的结晶仪式。
【标签】字标 / 描画 / 品牌
【画幅】1920×1080（16:9 横屏）
【时长】3.5 秒，动画确定性、首尾可循环

【视觉与设计规范】
大字距骨架字标全字符并行连续描画：同帧起笔、pathLength 归一保证同帧齐收成词——整词一体的结晶仪式。
【可改文案】字标（默认「MOTION」）、强调色（默认「#00FF00」）、背景色（默认「#0A0A0F」）、文字色（默认「#F4F4F2」）
